...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'DtYRk0mum5z_gOv',
      'isDeleted':false,
      'username':'Colleen_Schmeler90',
      'email':'Scot_Osinski14@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.updateOne(User, { 'username':'Colleen_Schmeler90' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'ud4tKGy6puHYG7a',
      'isDeleted':false,
      'username':'Frederique9',
      'email':'Kayden.Collins46@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.updateOne(User, { 'username':'Frederique9' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'CUG0Nq8uP1bt38v',
      'isDeleted':false,
      'username':'Hoyt16',
      'email':'Terry45@yahoo.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Teacher
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let teacher = await dbService.updateOne(User, { 'username':'Hoyt16' }, userToBeInserted,  { upsert: true });
...
async function seedRole () {
  try {
    const roles = [ 'Teacher', 'User', 'Admin', 'System_User' ];
